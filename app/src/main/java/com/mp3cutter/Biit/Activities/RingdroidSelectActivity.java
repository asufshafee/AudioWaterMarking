...
import com.mp3cutter.Biit.Adapters.SongsAdapter;
...
import com.mp3cutter.Biit.Views.FastScroller;
import com.mp3cutter.Biit.Player;
...
import static com.mp3cutter.Biit.Ringdroid.Constants.REQUEST_ID_MULTIPLE_PERMISSIONS;
import static com.mp3cutter.Biit.Ringdroid.Constants.REQUEST_ID_READ_CONTACTS_PERMISSION;
import static com.mp3cutter.Biit.Ringdroid.Constants.REQUEST_ID_RECORD_AUDIO_PERMISSION;
...
public class RingdroidSelectActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        File sdDirectory1 = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "Music");
        if (sdDirectory1.exists() && sdDirectory1.isDirectory()) {
            //directory is present, no need to do anything here
        ...
        if (getIntent().getStringExtra("Marked") != null) {

            mSongsAdapter = new SongsAdapter(this, mData, getIntent().getStringExtra("Marked"));
    ...
    private void loadData() {
        ...
        List<SongsModel> MyList=new ArrayList<>();

        if (getIntent().getStringExtra("Marked")!=null) {

            mData.addAll(Utils.getSongList(getApplicationContext(), true, null,"marked"));
//            mData.addAll(Utils.getSongList(getApplicationContext(), false, null,"marked"));

//            for (int position = 0; position < mData.size() - 1; position++) {
//                if (mData.get(position).mArtistName.contains("AsifShafee")) {
//                    MyList.add(mData.get(position));
//                }
//            }
//            mData.clear();
//            mData=MyList;
                ...
                mData.addAll(Utils.getSongList(getApplicationContext(), true, null,"no"));
                mData.addAll(Utils.getSongList(getApplicationContext(), false, null,"no"));
    ...
    private void confirmDelete(int pos) {
        ...
        if (artist.equals(ringdroidArtist)) {
            message = getResources().getText(
                    R.string.confirm_delete_Audio);
        } else {
            message = getResources().getText(
                    R.string.confirm_delete_non_Audio);
        }
...
//        getContentResolver().delete(Uri.parse(itemUri), null, null);
    ...
    private void startEditor(final int pos) {
        ...
        if (getIntent().getStringExtra("Marked")!=null)
        {
            ...
            bundle.putSerializable("Data",mData.get(pos));

            intent.putExtra("bundle",bundle);
            startActivity(intent);
        }else {

            final Dialog dialog = new Dialog(RingdroidSelectActivity.this);
            // inflate the layout
            dialog.setContentView(R.layout.conform_dialog);
            // Set the dialog text -- this is better done in the XML
            Button INsertMark;
            INsertMark = (Button) dialog.findViewById(R.id.Auto);
            ...
            Button Cancel;
            Cancel = (Button) dialog.findViewById(R.id.Manually);
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        if (getIntent().getStringExtra("Marked")!=null) {
            mData.addAll(Utils.getSongList(getApplicationContext(), true, newText,"marked"));
            mData.addAll(Utils.getSongList(getApplicationContext(), false, newText,"no"));
        }else {
            mData.addAll(Utils.getSongList(getApplicationContext(), true, newText,"no"));
            mData.addAll(Utils.getSongList(getApplicationContext(), false, newText,"no"));
        }
